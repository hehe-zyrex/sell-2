...
# ==================== CONFIGURATION ====================
BOT_TOKEN = "8913447240:AAFcmpRjKZWhCjKfNVzD4dE9p1jWVDiowgI"  # Replace with your bot token
ADMIN_IDS = [8896981303]  # Replace with admin user IDs
...
class RedeemBot:
    ...
    def setup_handlers(self):
        ...
        add_code_conv = ConversationHandler(
        ...
        self.application.add_handler(add_code_conv)
    ...
    async def callback_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if data == "profile":
                ...
                await self.show_referrals(query, context, user_id)
            elif data == "withdraw":
                await self.show_withdraw(query, user_id)
            elif data == "history":
                await self.show_history(query, user_id)
            elif data == "mycodes":
                await self.show_mycodes(query, user_id)
            elif data == "admin_panel":
                await self.show_admin(query, user_id)
            elif data == "back":
                await self.show_start(query, context, user_id)
            
            # Admin actions
            elif data.startswith("admin_"):
                await self.handle_admin_action(query, context, user_id, data)
            
            # Withdrawal actions
            elif data.startswith("withdraw_"):
                await self.handle_withdraw_action(query, context, user_id, data)
            
            # Code actions
            elif data.startswith("code_"):
                await self.handle_code_action(query, user_id, data)
    ...
    # ==================== SHOW METHODS ====================
    async def show_start(self, query, context, user_id):
    ...
    async def show_referrals(self, query, context, user_id):
    ...
    async def show_withdraw(self, query, user_id):
        user_data = self.db.get_user(user_id)
        if not user_data:
            await query.edit_message_text("⚠️ Please use /start.")
            return
    ...
    async def show_mycodes(self, query, user_id):
        codes = self.db.get_private_codes_for_user(user_id)
    ...
    async def show_admin(self, query, user_id):
    ...
    async def handle_admin_action(self, query, context, user_id, data):
        if user_id not in ADMIN_IDS:
            await query.edit_message_text("❌ Unauthorized.")
            return
        ...
        if action == "pending":
            await self.admin_pending(query)
        elif action == "addcode":
            ...
            await self.admin_listcodes(query)
        elif action == "credits":
            await query.edit_message_text(
                "💳 **Manage Credits**\n\n"
    ...
    async def handle_withdraw_action(self, query, context, user_id, data):
        parts = data.split("_")
        action = parts[1] if len(parts) > 1 else ""
    ...
    async def handle_code_action(self, query, user_id, data):
        parts = data.split("_")
        if len(parts) < 3:
            return
        ...
        code = parts[2]
        self.db.mark_code_as_viewed(code, user_id)
        await query.edit_message_text(f"🎟️ **Code**\n\n`{code}`\n\n🔒 Private.")

    # ==================== ADMIN FUNCTIONS ====================
    async def admin_pending(self, query):
    ...
    async def admin_listcodes(self, query):
